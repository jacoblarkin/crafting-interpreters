...
static void initCompiler(Compiler* compiler, FunctionType type) {
  ...
  current = compiler;
  if(type != TYPE_SCRIPT && type != TYPE_ANONFUN) {
    current->function->name = copyString(parser.previous.start,
                                         parser.previous.length);
  } else if(type == TYPE_ANONFUN) {
    char anon_name[13] = {0};
    sprintf(anon_name, "af%010d", anon_counter++);
    current->function->name = copyString(anon_name, 12);
  }

  Local* local = &current->locals[current->localCount++];
  local->depth = 0;
  local->isCaptured = false;
  if(type != TYPE_FUNCTION && type != TYPE_ANONFUN) {
...
static bool identifiersEqual(Token* a, Token* b) {
  ...
  return memcmp(a->start, b->start, (unsigned long)(a->length)) == 0;
}
...
static void addLocal(Token name) {
  if(current->localCount == UINT8_COUNT) {
    error("Too many local vairables in function.");
    return ;
  }
